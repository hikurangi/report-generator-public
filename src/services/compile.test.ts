...
test('Interpolates a name correctly', () => {
  const text = 'When reading a passage of text, {{REPORTEE_NAME}} also read accurately and at a good rate.'
  ...
  const actual = compile({ text, reportee })
...
test('Interpolates possessive names correctly', () => {
  const text = 'Next, {{REPORTEE_NAME_POSSESSIVE}} fluency reading a piece of text was assessed.'
